...
| **Score** | A predicted-success number for a path; how good a bet it is. Not fixed — it moves as the batch's state changes. |
| **Status** | The observed lifecycle state of a path (candidate, building, passed, …). |
| **Action** | What to do with a path next: build it, or cancel it. |
| **Limit** | A "how much" bound: the dependency limit, selection limit, and prioritization limit. Scales with build resources (and other signals). |
