...
| `record_failed` | Record-stage work could not be completed and was abandoned after exhausting primary retries. | Event retention time |
...
| Record DLQ | Retain `record_failed`, or the more specific `promotion_failed` when failure attribution identifies promotion, before abandoning the remaining record work. | Stable event IDs make history retention idempotent without replaying facts, bookmarks, promotion, or hooks. |
