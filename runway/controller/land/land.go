...
// Package land consumes committing land requests from Runway's land queue. A
// request asks Runway to apply an ordered sequence of land steps onto the land
// target and commit the result.
//
// The controller obtains a Lander for the request's land target, calls Land,
// and publishes the LandResult (with the produced revisions) to the land-signal
// queue. A land conflict is an expected outcome (ack + publish FAILED result),
...
// Controller handles land queue messages.
...
// Params are the parameters for creating a new land controller.
type Params struct {
	TopicKey      consumer.TopicKey
	ConsumerGroup string
...
// NewController creates a new land controller for the runway service.
func NewController(p Params) *Controller {
	return &Controller{
		logger:        p.Logger.Named("land_controller"),
		metricsScope:  p.Scope.SubScope("land_controller"),
...
// Process deserializes the land request, performs the committing land, and
...
func (c *Controller) Process(ctx context.Context, delivery consumer.Delivery) error {
	...
	c.logger.Infow("received land request",
		"id", request.Id,
	...
	result, err := m.Land(ctx, request)
	if err != nil {
		if !lander.IsTerminal(err) {
			metrics.NamedCounter(c.metricsScope, opName, "land_errors", 1)
			return fmt.Errorf("failed to land for %s: %w", request.GetId(), err)
		}
	...
	if err := c.publish(ctx, runwaymq.TopicKeyLandSignal, result, msg.PartitionKey); err != nil {
		metrics.NamedCounter(c.metricsScope, opName, "publish_errors", 1)
		return fmt.Errorf("failed to publish land result for %s: %w", request.GetId(), err)
	}
