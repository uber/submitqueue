...
func TestProcess_PublishesMergeConflictCheckSignal(t *testing.T) {
	ctrl := gomock.NewController(t)
	...
	mockMerger.EXPECT().
		CheckMergeability(gomock.Any(), gomock.Any()).
		Return(&runwaymq.MergeResult{
			Id:      testID,
			Outcome: runwaypb.Outcome_SUCCEEDED,
			Steps:   []*runwaymq.StepResult{{StepId: "step-1"}},
		}, nil)
	expectPublish(t, q, publisher, "merge-conflict-check-signal", func(result *runwaymq.MergeResult) {
...
func TestProcess_PublishesFailedSignalOnConflict(t *testing.T) {
	...
	mockMerger.EXPECT().CheckMergeability(gomock.Any(), gomock.Any()).Return(nil, merger.ErrConflict)
	expectPublish(t, q, publisher, "merge-conflict-check-signal", func(result *runwaymq.MergeResult) {
		assert.Equal(t, testID, result.Id)
...
func TestProcess_ReturnsErrorOnCheckError(t *testing.T) {
	ctrl := gomock.NewController(t)
	...
	mockMerger.EXPECT().CheckMergeability(gomock.Any(), gomock.Any()).Return(nil, errors.New("merge backend down"))
