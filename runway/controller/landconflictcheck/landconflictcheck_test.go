...
func newRegistry(t *testing.T, ctrl *gomock.Controller, publishErr error) (consumer.TopicRegistry, *queuemock.MockPublisher) {
	...
	registry, err := consumer.NewTopicRegistry([]consumer.TopicConfig{
		{Key: runwaymq.TopicKeyLandConflictCheckSignal, Name: "land-conflict-check-signal", Queue: q},
	})
	require.NoError(t, err)
	return registry, pub
}
...
func TestNewController(t *testing.T) {
	...
	assert.Equal(t, runwaymq.TopicKeyLandConflictCheck, controller.TopicKey())
	assert.Equal(t, "runway-landconflictcheck", controller.ConsumerGroup())
	assert.Equal(t, "land-conflict-check", controller.Name())
}
...
func TestProcess_Success(t *testing.T) {
	...
	m.EXPECT().CheckLandability(gomock.Any(), gomock.Any()).Return(expectedResult, nil)
	...
	assert.Equal(t, "land-conflict-check-signal", gotTopic)
...
func TestProcess_LandConflict(t *testing.T) {
	...
	m := landermock.NewMockLander(ctrl)
	m.EXPECT().CheckLandability(gomock.Any(), gomock.Any()).Return(nil, fmt.Errorf("conflict in foo.go: %w", lander.ErrConflict))
...
func TestProcess_InvalidRequest(t *testing.T) {
	...
	m := landermock.NewMockLander(ctrl)
	m.EXPECT().CheckLandability(gomock.Any(), gomock.Any()).Return(nil, fmt.Errorf("bad strategy: %w", lander.ErrInvalidRequest))
...
func TestProcess_LanderInfraError(t *testing.T) {
	...
	m := landermock.NewMockLander(ctrl)
	m.EXPECT().CheckLandability(gomock.Any(), gomock.Any()).Return(nil, fmt.Errorf("git timeout"))
