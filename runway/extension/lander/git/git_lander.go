...
// Package git is a Lander implementation backed by a local git checkout. It
// applies a LandRequest's ordered steps onto a target branch, honoring each
// step's land strategy, and (for a committing Land) pushes the result.
//
...
//
// Authorship: the committer is always the lander's configured identity, since
...
//
// Atomicity: for a committing land nothing reaches the remote until every step
...
// land then writes two things rather than one — the changes' head branches
...
// land first moves each change's head branch to the commit it became and then
...
//
// Dry-run (CheckLandability) applies the exact same steps but never pushes;
...
// GitRuntime identifies the explicitly provided Git runtime used by the Lander.
type GitRuntime struct {
	...
	// empty otherwise. Names that could alter land semantics do not belong
	// here — the scrubbed environment is what keeps the land reproducible.
	PassthroughEnv []string
}

// Params holds the dependencies for the git Lander.
type Params struct {
	// CheckoutPath is the absolute path to an existing git checkout that the
	// Lander will operate against. The Lander owns this working tree.
	CheckoutPath string
	...
	DefaultStrategy landstrategypb.Strategy
	// Runtime is the pinned Git runtime used for every invocation.
	Runtime GitRuntime
	// MaxPushAttempts caps how many times a committing land retries the full
	...
	// without the lander having to know the provider or call its API.
	//
	...
	// both leave the provider comparing against the pre-land head, and it
	...
	// lander was not asked to move is a surprise unless a deployment opted in.
	//
	// A head branch that cannot be moved fails the land before the target is
	...
	// without it a land of the wrong object fails loudly instead of quietly
...
// gitLander implements lander.Lander by shelling out to the `git` CLI against a
// local checkout.
type gitLander struct {
	checkoutPath     string
	remote           string
	target           string
	defaultStrategy  landstrategypb.Strategy
...
// NewLander constructs a git-backed Lander operating against the given checkout.
...
func NewLander(params Params) (lander.Lander, error) {
	...
	return &gitLander{
		...
		logger:                  params.Logger.Named("git_lander"),
		metricsScope:            params.MetricsScope.SubScope("git_lander"),
	}, nil
}
...
// CheckLandability applies the request's steps as a dry run: it verifies each
// step applies cleanly without committing to the remote, and returns per-step
// results with empty Outputs.
func (m *gitLander) CheckLandability(ctx context.Context, req *runwaymq.LandRequest) (*runwaymq.LandResult, error) {
	return m.process(ctx, req, false)
}

// Land applies the request's steps and commits the result to the remote,
// returning per-step Outputs (the revisions produced on the target).
func (m *gitLander) Land(ctx context.Context, req *runwaymq.LandRequest) (*runwaymq.LandResult, error) {
	return m.process(ctx, req, true)
}

// process is the shared entry point for both Land and CheckLandability. The
// commit flag selects whether the result is pushed to the remote (Land) or
// applied locally and discarded (CheckLandability).
func (m *gitLander) process(ctx context.Context, req *runwaymq.LandRequest, commit bool) (ret *runwaymq.LandResult, retErr error) {
	opName := "check"
	if commit {
		opName = "land"
	}
	...
	if steps[0].strategy == landstrategypb.Strategy_PROMOTE {
...
// composition rule. All failures here are terminal (lander.ErrInvalidRequest):
// retrying never succeeds, so the controller publishes a FAILED result rather
// than nacking.
func (m *gitLander) resolveAndValidate(req *runwaymq.LandRequest) ([]resolvedStep, error) {
	if len(req.GetSteps()) == 0 {
		return nil, fmt.Errorf("%w: request has no steps", lander.ErrInvalidRequest)
	}
	...
	for _, step := range req.GetSteps() {
		...
		if strategy == landstrategypb.Strategy_DEFAULT {
			strategy = m.defaultStrategy
		}
		if !isConcreteStrategy(strategy) {
			return nil, fmt.Errorf("%w: unsupported strategy %v", lander.ErrInvalidRequest, step.GetStrategy())
		}
		if strategy == landstrategypb.Strategy_PROMOTE {
			promoteSeen = true
		}

		ch := step.GetChange()
		if ch == nil || len(ch.GetUris()) == 0 {
			return nil, fmt.Errorf("%w: step %q has no change URIs", lander.ErrInvalidRequest, step.GetStepId())
		}
	...
	if promoteSeen {
		if len(resolved) != 1 ||
			...
			return nil, fmt.Errorf("%w: PROMOTE must be the entire request (one step, one change, one URI)", lander.ErrInvalidRequest)
		}
	}
...
//
// There is no sense in one land being addressed through two providers, and
...
func (o *providerCheck) check(ref changeRef, stepID string) error {
	...
	if ref.Provider != o.provider {
		return fmt.Errorf("%w: request mixes change providers: step %q uses %q, step %q uses %q",
			lander.ErrInvalidRequest, o.stepID, o.provider, stepID, ref.Provider)
	}
	return nil
}
...
func (m *gitLander) applyTransforming(ctx context.Context, req *runwaymq.LandRequest, steps []resolvedStep, commit bool) (*runwaymq.LandResult, error) {
	...
	for attempt := 1; attempt <= m.maxPushAttempts; attempt++ {
		...
		if err == nil {
			...
			m.logger.Debugw("land complete", "id", req.GetId(), "target", m.target, "commit", commit)
			return successResult(req, stepResults), nil
		}

		// A conflict is terminal — no retry. Discard any partial dry-run state.
		if errors.Is(err, lander.ErrConflict) {
			if !commit {
		...
		coremetrics.NamedCounter(m.metricsScope, "land", "stale_base_retries", 1)
		m.logger.Warnw("remote tip moved during land, retrying",
			"attempt", attempt,
	...
	coremetrics.NamedCounter(m.metricsScope, "land", "stale_base_giveup", 1)
	return nil, fmt.Errorf("exceeded %d land attempts due to remote contention: %w", m.maxPushAttempts, lastErr)
}
...
func (m *gitLander) tryApply(ctx context.Context, steps []resolvedStep, commit bool, tracked headBranchTracker) (string, []*runwaymq.StepResult, error) {
	if err := m.resetToRemote(ctx); err != nil {
		coremetrics.NamedCounter(m.metricsScope, "land", "reset_errors", 1)
		return "", nil, err
	}
	...
	if commit {
		...
		if err := m.push(ctx); err != nil {
			coremetrics.NamedCounter(m.metricsScope, "land", "git_push_errors", 1)
			return baseSHA, nil, err
		}
	}
...
func (m *gitLander) applySteps(ctx context.Context, steps []resolvedStep) ([]*runwaymq.StepResult, []headUpdate, error) {
	...
	for _, rs := range steps {
		...
		case landstrategypb.Strategy_MERGE:
			out, err = m.applyMergeCommit(ctx, rs)
		default:
			// resolveAndValidate rejects anything else; defensive.
			return nil, nil, fmt.Errorf("%w: unsupported strategy %v", lander.ErrInvalidRequest, rs.strategy)
		}
		if err != nil {
			return nil, nil, err
		}
...
func (m *gitLander) applyRebase(ctx context.Context, rs resolvedStep) (applied, error) {
...
func (m *gitLander) applySquashRebase(ctx context.Context, rs resolvedStep) (applied, error) {
	var out applied
...
// idempotent.
func (m *gitLander) squashChange(ctx context.Context, step *runwaymq.LandStep, ref changeRef) (string, bool, error) {
	...
	// Credit it to the change's own author rather than to the lander.
...
// applyMergeCommit creates a --no-ff merge commit for every change in the step,
...
func (m *gitLander) applyMergeCommit(ctx context.Context, rs resolvedStep) (applied, error) {
	var out applied
	for _, ref := range rs.refs {
		...
		// The merge commit is minted by the lander, so it would otherwise be
		...
		if err != nil {
			...
			return applied{}, m.classifyLandFailure(ref, o, conflicted)
		}
...
func (m *gitLander) promote(ctx context.Context, req *runwaymq.LandRequest, rs resolvedStep, commit bool) (*runwaymq.LandResult, error) {
	...
	for attempt := 1; attempt <= m.maxPushAttempts; attempt++ {
		...
		if !fastForward {
			return nil, fmt.Errorf("%w: promote target %s is not a fast-forward of %s", lander.ErrConflict, sha, tip)
		}
...
// classifyLandFailure decides what a failed `git merge` actually means, given
...
func (m *gitLander) classifyLandFailure(ref changeRef, out []byte, conflicted bool) error {
	detail := strings.TrimSpace(string(out))
	if strings.Contains(detail, "refusing to merge unrelated histories") {
		coremetrics.NamedCounter(m.metricsScope, "land", "unrelated_histories", 1)
		return fmt.Errorf("%w: %s shares no history with the land target; integrating an imported history requires the lander to allow unrelated histories",
			lander.ErrInvalidRequest, ref.Label)
	}
	if !conflicted {
		coremetrics.NamedCounter(m.metricsScope, "land", "land_errors", 1)
		return fmt.Errorf("git merge %s: %s", ref.SHA, detail)
	}
	coremetrics.NamedCounter(m.metricsScope, "land", "land_conflicts", 1)
	return fmt.Errorf("%w: git merge %s: %s", lander.ErrConflict, ref.SHA, detail)
}
...
func (m *gitLander) pickStepChanges(ctx context.Context, rs resolvedStep) ([]string, []headUpdate, error) {
	var picked []string
	var heads []headUpdate
	for _, ref := range rs.refs {
		created, err := m.pickRange(ctx, ref)
...
func (m *gitLander) pickRange(ctx context.Context, ref changeRef) ([]string, error) {
	...
	if base == "" {
		...
		return nil, fmt.Errorf("%w: change %s shares no history with the land target; use the MERGE strategy to integrate an unrelated history",
			lander.ErrInvalidRequest, ref.Label)
	}
...
func (m *gitLander) cherryPickRange(ctx context.Context, base, head string) error {
	...
	for err != nil {
		...
		detail := strings.TrimSpace(string(out))
		if !conflicted {
			coremetrics.NamedCounter(m.metricsScope, "land", "cherry_pick_errors", 1)
			return fmt.Errorf("git cherry-pick %s..%s: %w: %s", base, head, err, detail)
		}
		coremetrics.NamedCounter(m.metricsScope, "land", "cherry_pick_conflicts", 1)
		return fmt.Errorf("%w: git cherry-pick %s..%s: %s", lander.ErrConflict, base, head, detail)
	}
	return nil
}
...
func (m *gitLander) hasUnmergedPaths(ctx context.Context) bool {
...
// a failure.
func (m *gitLander) mergeBase(ctx context.Context, a, b string) (string, error) {
...
// drops some of them.
func (m *gitLander) revList(ctx context.Context, from, to string) ([]string, error) {
...
func (m *gitLander) resetToRemote(ctx context.Context) error {
...
func (m *gitLander) headSHA(ctx context.Context) (string, error) {
...
func (m *gitLander) refetchTipSHA(ctx context.Context) (string, error) {
	if _, err := m.run(ctx, nil, "fetch", m.remote); err != nil {
		return "", fmt.Errorf("git fetch %s: %w", m.remote, err)
	}
	remoteRef := m.remote + "/" + m.target
...
func (m *gitLander) isAncestor(ctx context.Context, ancestor, descendant string) (bool, error) {
...
func (m *gitLander) commitTreeSHA(ctx context.Context, ref string) (string, error) {
...
func (m *gitLander) push(ctx context.Context) error {
...
func (m *gitLander) run(ctx context.Context, stdin []byte, args ...string) ([]byte, error) {
...
func (m *gitLander) runAs(ctx context.Context, author authorIdent, stdin []byte, args ...string) ([]byte, error) {
...
func (m *gitLander) runCombined(ctx context.Context, stdin []byte, args ...string) ([]byte, error) {
...
// the given author.
func (m *gitLander) runCombinedAs(ctx context.Context, author authorIdent, stdin []byte, args ...string) ([]byte, error) {
	cmd := m.commandAs(ctx, author, args...)
	if stdin != nil {
		cmd.Stdin = bytes.NewReader(stdin)
	}
...
func (m *gitLander) command(ctx context.Context, args ...string) *exec.Cmd {
...
func (m *gitLander) commandAs(ctx context.Context, author authorIdent, args ...string) *exec.Cmd {
...
func isConcreteStrategy(s landstrategypb.Strategy) bool {
	switch s {
	case landstrategypb.Strategy_REBASE,
		landstrategypb.Strategy_SQUASH_REBASE,
		landstrategypb.Strategy_MERGE,
...
// silently omitted.
func squashMessage(step *runwaymq.LandStep, ref changeRef) string {
	if step.GetStepId() != "" {
...
// check, which reports landability only.
...
// successResult builds a SUCCEEDED LandResult echoing the request id.
func successResult(req *runwaymq.LandRequest, steps []*runwaymq.StepResult) *runwaymq.LandResult {
...
// promoteResult builds a SUCCEEDED LandResult for a promote. A committing
// promote reports the promoted SHA as the step's single output; a dry-run check
// reports no output.
func promoteResult(req *runwaymq.LandRequest, rs resolvedStep, sha string, commit bool) *runwaymq.LandResult {
