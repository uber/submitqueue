...
// the Lander operates on. Tests run real `git` commands so they exercise the
// same code path as production.
//
// The fixture also exposes helpers for pushing additional commits to side
// branches on the remote, so each test can build the SHAs it needs the Lander
// to apply.
...
func setupGitFixture(t *testing.T) gitFixture {
	...
	// Seed main with one initial commit so the Lander's reset/fetch flow has
...
func TestCherryPickRange_NonConflictFailureIsRetryable(t *testing.T) {
	...
	f := setupGitFixture(t)
	m := f.newLander(t, landstrategypb.Strategy_REBASE).(*gitLander)

	err := m.cherryPickRange(context.Background(), "notarevision", "alsonotarevision")
	require.Error(t, err)
	assert.False(t, errors.Is(err, lander.ErrConflict), "a non-conflict failure must stay retryable")
	assert.False(t, errors.Is(err, lander.ErrInvalidRequest))
}
...
func TestCherryPickRange_RealConflictIsErrConflict(t *testing.T) {
	...
	require.Error(t, err)
	assert.True(t, errors.Is(err, lander.ErrConflict))
	assert.True(t, f.checkoutClean(t), "the aborted pick must leave no conflicted index behind")
}

// --- REBASE ---

func TestLand_Rebase_SingleChangeSingleURI(t *testing.T) {
	f := setupGitFixture(t)
	sha := f.pushPRCommit(t, "feature/a", "hello.txt", "hello\nearth\n", "tweak hello")
	m := f.newLander(t, landstrategypb.Strategy_REBASE)

	res, err := m.Land(context.Background(), req("b", stepOf(landstrategypb.Strategy_REBASE, "s1", uri(sha))))
	require.NoError(t, err)
...
func TestLand_Rebase_StackedURIsInOneChange(t *testing.T) {
	f := setupGitFixture(t)
	sha1, sha2 := f.pushStack(t)

	m := f.newLander(t, landstrategypb.Strategy_REBASE)
	res, err := m.Land(context.Background(), req("b",
		stepOf(landstrategypb.Strategy_REBASE, "s1", uri(sha1), uri(sha2)),
	))
	require.NoError(t, err)
	require.Len(t, res.GetSteps(), 1)
...
func TestLand_Rebase_AlreadyLandedChange(t *testing.T) {
	f := setupGitFixture(t)
	sha := f.pushPRCommit(t, "feature/a", "hello.txt", "hello\nearth\n", "tweak hello")

	// Land the same content on main outside the Lander so the cherry-pick
	...
	assert.Equal(t, mainBefore, f.remoteHEAD(t),
		"rebased-out land should not advance the remote tip")
}

func TestLand_Rebase_MixedChanges(t *testing.T) {
	t.Run("two steps", func(t *testing.T) {
		...
		m := f.newLander(t, landstrategypb.Strategy_REBASE)
		res, err := m.Land(context.Background(), req("b",
			stepOf(landstrategypb.Strategy_REBASE, "s1", uri(subsumedSHA)),
			stepOf(landstrategypb.Strategy_REBASE, "s2", uri(freshSHA)),
		))
		require.NoError(t, err)
	...
	t.Run("one change two URIs", func(t *testing.T) {
		...
		m := f.newLander(t, landstrategypb.Strategy_REBASE)
		res, err := m.Land(context.Background(), req("b",
			stepOf(landstrategypb.Strategy_REBASE, "s1", uri(subsumedSHA), uri(freshSHA)),
		))
		require.NoError(t, err)
		require.Len(t, res.GetSteps(), 1)
...
func TestLand_Rebase_Conflict(t *testing.T) {
	f := setupGitFixture(t)
	mainBefore, conflictingSHA := f.setupConflict(t)

	m := f.newLander(t, landstrategypb.Strategy_REBASE)
	_, err := m.Land(context.Background(), req("b", stepOf(landstrategypb.Strategy_REBASE, "s1", uri(conflictingSHA))))
	require.Error(t, err)
	assert.True(t, errors.Is(err, lander.ErrConflict))
	assert.Equal(t, mainBefore, f.remoteHEAD(t), "on conflict the remote tip must not move")
}

func TestLand_Rebase_ResetsBetweenCalls(t *testing.T) {
	f := setupGitFixture(t)
	sha := f.pushPRCommit(t, "feature/a", "hello.txt", "hello\nearth\n", "tweak hello")
	m := f.newLander(t, landstrategypb.Strategy_REBASE)
...
func TestLand_Rebase_RecoversAfterPriorConflict(t *testing.T) {
	f := setupGitFixture(t)
	...
	// A subsequent, clean land must succeed even though the prior call left a
	// cherry-pick in progress before its rollback.
	freshSHA := f.pushPRCommit(t, "feature/c", "extra.txt", "extra\n", "add extra")
	res, err := m.Land(context.Background(), req("b2", stepOf(landstrategypb.Strategy_REBASE, "s1", uri(freshSHA))))
	require.NoError(t, err)
	assert.Equal(t, runwaypb.Outcome_SUCCEEDED, res.GetOutcome())
	assert.Equal(t, "extra\n", f.remoteFile(t, "extra.txt"))
}

func TestLand_Rebase_RetriesWhenRemoteMovesUnderUs(t *testing.T) {
	f := setupGitFixture(t)
	...
	m := f.newLander(t, landstrategypb.Strategy_REBASE)
	res, err := m.Land(context.Background(), req("b", stepOf(landstrategypb.Strategy_REBASE, "s1", uri(featureSHA))))
	require.NoError(t, err)
	require.Len(t, res.GetSteps(), 1)
	require.Len(t, res.GetSteps()[0].GetOutputs(), 1)
...
func TestLand_Rebase_GivesUpAfterMaxAttempts(t *testing.T) {
	f := setupGitFixture(t)
	raceSHAs := []string{
	...
	m, err := NewLander(Params{
		CheckoutPath:    f.checkoutDir,
		Remote:          "origin",
		Target:          "main",
		DefaultStrategy: landstrategypb.Strategy_REBASE,
		Runtime:         testGitRuntime(t),
		MaxPushAttempts: 2,
	...
	_, err = m.Land(context.Background(), req("b", stepOf(landstrategypb.Strategy_REBASE, "s1", uri(featureSHA))))
	require.Error(t, err)
	assert.False(t, errors.Is(err, lander.ErrConflict))
...
func TestLand_SquashRebase_OneCommitPerChange(t *testing.T) {
	...
	m := f.newLander(t, landstrategypb.Strategy_REBASE)
	res, err := m.Land(context.Background(), req("b",
		stepOf(landstrategypb.Strategy_SQUASH_REBASE, "s1", uri(sha1), uri(sha2)),
	))
	require.NoError(t, err)
	assert.Equal(t, runwaypb.Outcome_SUCCEEDED, res.GetOutcome())
	require.Len(t, res.GetSteps(), 1)
...
func TestLand_SquashRebase_AlreadyLanded(t *testing.T) {
	f := setupGitFixture(t)
	sha := f.pushPRCommit(t, "feature/a", "hello.txt", "hello\nearth\n", "tweak hello")
	f.landOnMain(t, sha)
	mainBefore := f.remoteHEAD(t)
	...
	m := f.newLander(t, landstrategypb.Strategy_REBASE)
	res, err := m.Land(context.Background(), req("b", stepOf(landstrategypb.Strategy_SQUASH_REBASE, "s1", uri(sha))))
	require.NoError(t, err)
	require.Len(t, res.GetSteps(), 1)
	assert.Empty(t, res.GetSteps()[0].GetOutputs())
	assert.Equal(t, mainBefore, f.remoteHEAD(t))
...
func TestLand_Merge_FreshChangeCreatesMergeCommit(t *testing.T) {
	f := setupGitFixture(t)
	freshSHA := f.pushPRCommit(t, "feature/a", "hello.txt", "hello\nearth\n", "tweak hello")

	m := f.newLander(t, landstrategypb.Strategy_REBASE)
	res, err := m.Land(context.Background(), req("b", stepOf(landstrategypb.Strategy_MERGE, "s1", uri(freshSHA))))
	require.NoError(t, err)
	assert.Equal(t, runwaypb.Outcome_SUCCEEDED, res.GetOutcome())
	require.Len(t, res.GetSteps(), 1)
	require.Len(t, res.GetSteps()[0].GetOutputs(), 1)
...
func TestLand_Merge_AlreadyAncestor(t *testing.T) {
	f := setupGitFixture(t)
	freshSHA := f.pushPRCommit(t, "feature/a", "hello.txt", "hello\nearth\n", "tweak hello")
...
func TestLand_Promote_MultiCommitChange(t *testing.T) {
	...
	m := f.newLander(t, landstrategypb.Strategy_REBASE)
	res, err := m.Land(context.Background(), req("b", stepOf(landstrategypb.Strategy_PROMOTE, "s1", uri(head))))
	require.NoError(t, err)
...
func TestLand_Promote_UnavailableCommitIsInvalidNotRetryable(t *testing.T) {
	...
	_, err := m.Land(context.Background(), req("b", stepOf(landstrategypb.Strategy_PROMOTE, "s1", uri(fakeSHA))))
	require.Error(t, err)
	assert.True(t, errors.Is(err, lander.ErrInvalidRequest))
	assert.False(t, errors.Is(err, lander.ErrConflict))
}

func TestLand_Promote_FastForward(t *testing.T) {
	f := setupGitFixture(t)
	ffSHA := f.pushPRCommit(t, "feature/ff", "hello.txt", "hello\nearth\n", "ff")

	m := f.newLander(t, landstrategypb.Strategy_REBASE)
	res, err := m.Land(context.Background(), req("b", stepOf(landstrategypb.Strategy_PROMOTE, "s1", uri(ffSHA))))
	require.NoError(t, err)
	assert.Equal(t, runwaypb.Outcome_SUCCEEDED, res.GetOutcome())
	require.Len(t, res.GetSteps(), 1)
	require.Len(t, res.GetSteps()[0].GetOutputs(), 1)
...
func TestLand_Promote_AlreadyContained(t *testing.T) {
	f := setupGitFixture(t)
	...
	m := f.newLander(t, landstrategypb.Strategy_REBASE)
	res, err := m.Land(context.Background(), req("b", stepOf(landstrategypb.Strategy_PROMOTE, "s1", uri(seedSHA))))
	require.NoError(t, err)
	assert.Equal(t, runwaypb.Outcome_SUCCEEDED, res.GetOutcome())
	assert.Equal(t, mainBefore, f.remoteHEAD(t), "promoting an already-contained SHA does not move the tip")
}

func TestLand_Promote_Divergent(t *testing.T) {
	f := setupGitFixture(t)
	seedSHA := f.remoteSHA(t, "main")
	...
	m := f.newLander(t, landstrategypb.Strategy_REBASE)
	_, err := m.Land(context.Background(), req("b", stepOf(landstrategypb.Strategy_PROMOTE, "s1", uri(divSHA))))
	require.Error(t, err)
	assert.True(t, errors.Is(err, lander.ErrConflict))
	assert.Equal(t, mainBefore, f.remoteHEAD(t))
}
...
func TestLand_Default_ResolvesToRebase(t *testing.T) {
	f := setupGitFixture(t)
	sha := f.pushPRCommit(t, "feature/a", "hello.txt", "hello\nearth\n", "tweak hello")

	m := f.newLander(t, landstrategypb.Strategy_REBASE)
	res, err := m.Land(context.Background(), req("b", stepOf(landstrategypb.Strategy_DEFAULT, "s1", uri(sha))))
	require.NoError(t, err)
	assert.Equal(t, runwaypb.Outcome_SUCCEEDED, res.GetOutcome())
	require.Len(t, res.GetSteps(), 1)
...
func TestClassifyLandFailure(t *testing.T) {
	f := setupGitFixture(t)
	m := f.newLander(t, landstrategypb.Strategy_REBASE).(*gitLander)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			err := m.classifyLandFailure(ref, []byte(tt.out), tt.conflicted)
			require.Error(t, err)
			assert.Equal(t, tt.wantConflict, errors.Is(err, lander.ErrConflict))
			assert.Equal(t, tt.wantInvalid, errors.Is(err, lander.ErrInvalidRequest))
		})
	}
}
...
// newUnrunnableLander builds a Lander whose git executable does not exist, so
// any git invocation fails loudly. A request rejected by this Lander with
...
func (f gitFixture) newUnrunnableLander(t *testing.T) lander.Lander {
	t.Helper()
	return f.newLanderWith(t, func(p *Params) {
		p.Runtime.Executable = filepath.Join(t.TempDir(), "no-such-git")
	})
}

func TestLand_RejectsInconsistentProvider(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			req: req("b",
				stepOf(landstrategypb.Strategy_REBASE, "s1", "github://github.example.com/uber/one/pull/1/"+fakeSHA),
				stepOf(landstrategypb.Strategy_REBASE, "s2", "git://git.example.com/uber/one/refs%2Fheads%2Fmain/"+otherSHA),
			),
		},
		{
			name: "one change spanning two providers",
			req: req("b", stepOf(landstrategypb.Strategy_REBASE, "s1",
		...
		},
		{
			...
			req:  req("b", stepOf(landstrategypb.Strategy_REBASE, "s1", "phab://phab.example.com/D123/456")),
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			f := setupGitFixture(t)
			m := f.newUnrunnableLander(t)

			_, err := m.Land(context.Background(), tt.req)
			require.Error(t, err)
			assert.True(t, errors.Is(err, lander.ErrInvalidRequest),
...
func TestLand_AcceptsMultipleChangesFromOneProvider(t *testing.T) {
	...
	m := f.newLander(t, landstrategypb.Strategy_REBASE)
	res, err := m.Land(context.Background(), req("b",
		stepOf(landstrategypb.Strategy_REBASE, "s1", uri(a), uri(b)),
		stepOf(landstrategypb.Strategy_REBASE, "s2", uri(c)),
	))
	require.NoError(t, err)
	assert.Equal(t, runwaypb.Outcome_SUCCEEDED, res.GetOutcome())
...
func TestLand_Merge_ImportsUnrelatedHistory(t *testing.T) {
	f := setupGitFixture(t)
	...
	m := f.newLanderWith(t, func(p *Params) { p.AllowUnrelatedHistories = true })
	res, err := m.Land(context.Background(), req("b", stepOf(landstrategypb.Strategy_MERGE, "s1", uri(head))))
	require.NoError(t, err)
	assert.Equal(t, runwaypb.Outcome_SUCCEEDED, res.GetOutcome())
...
func TestLand_Merge_ImportedHistoryRedeliveryIsNoOp(t *testing.T) {
	f := setupGitFixture(t)
	...
	m := f.newLanderWith(t, func(p *Params) { p.AllowUnrelatedHistories = true })
	request := req("b", stepOf(landstrategypb.Strategy_MERGE, "s1", uri(head)))
	_, err := m.Land(context.Background(), request)
	require.NoError(t, err)
	afterFirst := f.remoteHEAD(t)

	res, err := m.Land(context.Background(), request)
	require.NoError(t, err)
...
func TestLand_Merge_UnrelatedHistoryRejectedByDefault(t *testing.T) {
	// Without the option the merge is refused. It must not be reported as a
	// conflict: nothing collided, the lander simply declined to join two
	// unrelated graphs.
	f := setupGitFixture(t)
...
func TestLand_Rebase_UnrelatedHistoryRejected(t *testing.T) {
	...
	for _, strategy := range []landstrategypb.Strategy{
		landstrategypb.Strategy_REBASE,
		landstrategypb.Strategy_SQUASH_REBASE,
	} {
		t.Run(strategy.String(), func(t *testing.T) {
			m := f.newLanderWith(t, func(p *Params) { p.AllowUnrelatedHistories = true })
			_, err := m.Land(context.Background(), req("b", stepOf(strategy, "s1", uri(head))))
			require.Error(t, err)
			assert.True(t, errors.Is(err, lander.ErrInvalidRequest))
			assert.False(t, errors.Is(err, lander.ErrConflict))
		})
...
func TestLand_SquashRebase_MultiCommitChangeSingleURI(t *testing.T) {
	f := setupGitFixture(t)
...
func TestLand_Merge_MultiCommitChangeSingleURI(t *testing.T) {
	f := setupGitFixture(t)
	head := f.pushMultiCommitPR(t, "feature/multi",
		commitSpec{"a.txt", "a\n", "add a"},
		commitSpec{"b.txt", "b\n", "add b"},
...
func TestLand_Rebase_MultiCommitChangeSingleURI(t *testing.T) {
	f := setupGitFixture(t)
	head := f.pushMultiCommitPR(t, "feature/multi",
		commitSpec{"a.txt", "a\n", "add a"},
		commitSpec{"b.txt", "b\n", "add b"},
...
func TestLand_Rebase_MultiCommitChangeSharedFile(t *testing.T) {
...
func TestLand_Rebase_PartiallyLandedChange(t *testing.T) {
...
func TestLand_Rebase_EmptyCommitInRange(t *testing.T) {
	f := setupGitFixture(t)
...
func TestLand_Rebase_ConflictInRangeLeavesCheckoutUsable(t *testing.T) {
	f := setupGitFixture(t)
	head := f.pushMultiCommitPR(t, "feature/conflicting",
		commitSpec{"a.txt", "a\n", "add a"},
		commitSpec{"hello.txt", "hello\nmars\n", "diverge hello"},
	)
	// Land a conflicting edit to the same file outside the lander.
	...
	_, err = m.Land(context.Background(), req("c", stepOf(landstrategypb.Strategy_REBASE, "s2", uri(clean))))
	require.NoError(t, err)
}

func TestLand_Rebase_StackedMultiCommitChanges(t *testing.T) {
	...
	m := f.newLander(t, landstrategypb.Strategy_REBASE)
	res, err := m.Land(context.Background(), req("b",
		stepOf(landstrategypb.Strategy_REBASE, "s1", uri(aHead)),
		stepOf(landstrategypb.Strategy_REBASE, "s2", uri(bHead)),
	))
	require.NoError(t, err)
	require.Len(t, res.GetSteps(), 2)
...
func TestLand_SquashRebase_CreditsChangeAuthor(t *testing.T) {
	// A squash mints a fresh commit, so without attribution it would be
	// authored by the lander. The person who wrote the change should be its
	// author; the lander stays the committer, since it is what applied it.
	f := setupGitFixture(t)
	sha := f.pushPRCommit(t, "feature/a", "hello.txt", "hello\nearth\n", "tweak hello")

	m := f.newLander(t, landstrategypb.Strategy_REBASE)
	res, err := m.Land(context.Background(), req("b",
		stepOf(landstrategypb.Strategy_SQUASH_REBASE, "s1", uri(sha)),
	))
	require.NoError(t, err)
...
func TestLand_Merge_CreditsChangeAuthorOnMergeCommit(t *testing.T) {
	...
	m := f.newLander(t, landstrategypb.Strategy_REBASE)
	res, err := m.Land(context.Background(), req("b",
		stepOf(landstrategypb.Strategy_MERGE, "s1", uri(sha)),
	))
	require.NoError(t, err)
	require.Equal(t, runwaypb.Outcome_SUCCEEDED, res.GetOutcome())
...
func TestLand_SquashRebase_CreditsAuthorOfHeadCommit(t *testing.T) {
	...
	m := f.newLander(t, landstrategypb.Strategy_REBASE)
	res, err := m.Land(context.Background(), req("b",
		stepOf(landstrategypb.Strategy_SQUASH_REBASE, "s1", uri(head)),
	))
	require.NoError(t, err)
	require.Equal(t, runwaypb.Outcome_SUCCEEDED, res.GetOutcome())
...
func TestLand_Rebase_PreservesEachCommitsOwnAuthor(t *testing.T) {
	...
	m := f.newLander(t, landstrategypb.Strategy_REBASE)
	res, err := m.Land(context.Background(), req("b",
		stepOf(landstrategypb.Strategy_REBASE, "s1", uri(head)),
	))
	require.NoError(t, err)
	require.Equal(t, runwaypb.Outcome_SUCCEEDED, res.GetOutcome())
...
func TestLand_UnavailableCommitIsInvalidRequestNotConflict(t *testing.T) {
	// A commit the remote cannot supply is a request we can never satisfy. It
	// must not be reported as a land conflict: that would tell the client its
	// change conflicts when nothing was ever applied.
	f := setupGitFixture(t)
	m := f.newLander(t, landstrategypb.Strategy_REBASE)

	_, err := m.Land(context.Background(), req("b", stepOf(landstrategypb.Strategy_REBASE, "s1", uri(fakeSHA))))
	require.Error(t, err)
	assert.True(t, errors.Is(err, lander.ErrInvalidRequest))
	assert.False(t, errors.Is(err, lander.ErrConflict))
}

func TestLand_ChangeReachableOnlyViaPRRef(t *testing.T) {
...
func TestLand_StaleChangeRejected(t *testing.T) {
	f := setupGitFixture(t)
	...
	m := f.newLanderWith(t, func(p *Params) { p.CheckStaleness = true })
	_, err := m.Land(context.Background(), req("b", stepOf(landstrategypb.Strategy_REBASE, "s1", uri(stale))))
	require.Error(t, err)
	assert.True(t, errors.Is(err, lander.ErrInvalidRequest))
...
func TestLand_StalenessCheckOffByDefault(t *testing.T) {
	f := setupGitFixture(t)
	stale := f.pushPRCommit(t, "feature/s", "s.txt", "v1\n", "v1")
	f.publishPRRef(t, 1, stale)
...
func TestLand_InvalidRequests(t *testing.T) {
	tests := []struct {
		name string
		req  *runwaymq.LandRequest
	}{
		...
		},
		{
			...
			req: req("b",
				stepOf(landstrategypb.Strategy_PROMOTE, "s1", uri(fakeSHA)),
				stepOf(landstrategypb.Strategy_PROMOTE, "s2", uri(fakeSHA)),
			),
		},
		{
			name: "promote step with two URIs",
			req:  req("b", stepOf(landstrategypb.Strategy_PROMOTE, "s1", uri(fakeSHA), uri(fakeSHA))),
		},
		{
			name: "malformed URI",
			req:  req("b", stepOf(landstrategypb.Strategy_REBASE, "s1", "not a uri")),
		},
	}
...
// --- CheckLandability (dry-run) ---

func TestCheckLandability_RebaseMergeable(t *testing.T) {
	f := setupGitFixture(t)
	sha := f.pushPRCommit(t, "feature/a", "hello.txt", "hello\nearth\n", "tweak hello")
	mainBefore := f.remoteHEAD(t)

	m := f.newLander(t, landstrategypb.Strategy_REBASE)
	res, err := m.CheckLandability(context.Background(), req("b", stepOf(landstrategypb.Strategy_REBASE, "s1", uri(sha))))
	require.NoError(t, err)
	assert.Equal(t, runwaypb.Outcome_SUCCEEDED, res.GetOutcome())
	require.Len(t, res.GetSteps(), 1)
...
func TestCheckLandability_MergeMergeable(t *testing.T) {
	f := setupGitFixture(t)
	sha := f.pushPRCommit(t, "feature/a", "hello.txt", "hello\nearth\n", "tweak hello")
	mainBefore := f.remoteHEAD(t)

	m := f.newLander(t, landstrategypb.Strategy_REBASE)
	res, err := m.CheckLandability(context.Background(), req("b", stepOf(landstrategypb.Strategy_MERGE, "s1", uri(sha))))
	require.NoError(t, err)
	assert.Equal(t, runwaypb.Outcome_SUCCEEDED, res.GetOutcome())
	require.Len(t, res.GetSteps(), 1)
	assert.Empty(t, res.GetSteps()[0].GetOutputs())
...
func TestCheckLandability_Conflict(t *testing.T) {
	f := setupGitFixture(t)
	mainBefore, conflictingSHA := f.setupConflict(t)

	m := f.newLander(t, landstrategypb.Strategy_REBASE)
	_, err := m.CheckLandability(context.Background(), req("b", stepOf(landstrategypb.Strategy_REBASE, "s1", uri(conflictingSHA))))
	require.Error(t, err)
	assert.True(t, errors.Is(err, lander.ErrConflict))
	assert.Equal(t, mainBefore, f.remoteHEAD(t))
}

func TestCheckLandability_PromoteFastForward(t *testing.T) {
	f := setupGitFixture(t)
	ffSHA := f.pushPRCommit(t, "feature/ff", "hello.txt", "hello\nearth\n", "ff")
	mainBefore := f.remoteHEAD(t)

	m := f.newLander(t, landstrategypb.Strategy_REBASE)
	res, err := m.CheckLandability(context.Background(), req("b", stepOf(landstrategypb.Strategy_PROMOTE, "s1", uri(ffSHA))))
	require.NoError(t, err)
	assert.Equal(t, runwaypb.Outcome_SUCCEEDED, res.GetOutcome())
	require.Len(t, res.GetSteps(), 1)
	assert.Empty(t, res.GetSteps()[0].GetOutputs())
...
func TestCheckLandability_PromoteDivergent(t *testing.T) {
	f := setupGitFixture(t)
	seedSHA := f.remoteSHA(t, "main")
	divSHA := f.pushPRCommitFrom(t, seedSHA, "feature/div", "div.txt", "div\n", "div")
	otherSHA := f.pushPRCommitFrom(t, seedSHA, "feature/other", "other.txt", "other\n", "other")
	...
	m := f.newLander(t, landstrategypb.Strategy_REBASE)
	_, err := m.CheckLandability(context.Background(), req("b", stepOf(landstrategypb.Strategy_PROMOTE, "s1", uri(divSHA))))
	require.Error(t, err)
	assert.True(t, errors.Is(err, lander.ErrConflict))
	assert.Equal(t, mainBefore, f.remoteHEAD(t))
}
...
// req builds a LandRequest with the given correlation id and ordered steps.
func req(id string, steps ...*runwaymq.LandStep) *runwaymq.LandRequest {
	return &runwaymq.LandRequest{Id: id, Steps: steps}
}

// stepOf builds a step whose change carries the given URIs.
func stepOf(strategy landstrategypb.Strategy, stepID string, uris ...string) *runwaymq.LandStep {
...
// newLanderWith builds a Lander with non-default Params, for the options that
// only some tests exercise.
func (f gitFixture) newLanderWith(t *testing.T, mutate func(*Params)) lander.Lander {
	t.Helper()
	p := Params{
		CheckoutPath:    f.checkoutDir,
		Remote:          "origin",
...
// checkoutClean reports whether the lander's checkout has no in-progress
...
func (f gitFixture) newLander(t *testing.T, defaultStrategy landstrategypb.Strategy) lander.Lander {
	t.Helper()
	m, err := NewLander(Params{
		CheckoutPath:    f.checkoutDir,
		Remote:          "origin",
...
// setupConflict lands "earth" on main and returns the pre-land main SHA plus a
...
// uri builds a github-format URI ending in `sha` so the Lander's parser resolves
// it to that SHA.
...
// through.
//
// Combined with the gitLander's contention-detection (refetch + compare base
...
func (f gitFixture) installRaceHook(t *testing.T, raceSHAs []string) {
	...
	const script = `#!/bin/sh
# Contend only on the target branch. A land that moves change head branches
...
// formatted "Name <email>". The two differ whenever the lander creates a commit
