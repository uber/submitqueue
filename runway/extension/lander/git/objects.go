...
// apply paths, from a land conflict.
func (m *gitLander) ensureObjects(ctx context.Context, refs []changeRef) error {
...
func (m *gitLander) ensureObject(ctx context.Context, ref changeRef) error {
	...
	coremetrics.NamedCounter(m.metricsScope, "land", "object_unavailable", 1)
	...
	return fmt.Errorf("%w: commit %s of %s is not available from remote %s (tried by SHA and via %q)",
		lander.ErrInvalidRequest, ref.SHA, ref.Label, m.remote, ref.Ref)
}

// hasCommit reports whether sha names a commit object in the local checkout.
func (m *gitLander) hasCommit(ctx context.Context, sha string) bool {
	_, err := m.run(ctx, nil, "cat-file", "-e", sha+"^{commit}")
	return err == nil
}

// checkStale reports whether any change has been superseded since its URI was
// minted. Fetching by SHA guarantees the lander applies exactly the commit the
...
func (m *gitLander) checkStale(ctx context.Context, refs []changeRef) error {
	if !m.checkStaleness {
		return nil
	}
	for _, ref := range refs {
		...
		if current := fields[0]; current != ref.SHA {
			coremetrics.NamedCounter(m.metricsScope, "land", "stale_changes", 1)
			return fmt.Errorf("%w: change is stale: %s names commit %s but %s now points at %s",
				lander.ErrInvalidRequest, ref.Label, ref.SHA, ref.Ref, current)
		}
	}
	return nil
}
