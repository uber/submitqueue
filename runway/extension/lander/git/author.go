...
//
// Git records the two separately and the lander keeps them separate: the
// lander is always the committer, because it is what applied the change, while
...
// the request names rather than anything the lander infers. It is already
...
// failing the land over attribution.
func (m *gitLander) commitAuthor(ctx context.Context, sha string) (authorIdent, error) {
