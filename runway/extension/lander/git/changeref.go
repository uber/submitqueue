...
// changeRef is everything the lander needs from a change URI, reduced to a
...
type changeRef struct {
	...
	// commit messages the lander synthesizes.
	Label string
}

// resolveChange maps a change URI onto the lander's provider-neutral view of
...
func resolveChange(uri string) (changeRef, error) {
	...
	if !ok {
		return changeRef{}, fmt.Errorf("%w: change URI %q has no scheme", lander.ErrInvalidRequest, uri)
	}
	...
	case "github":
		...
		if err != nil {
			return changeRef{}, fmt.Errorf("%w: invalid change URI %q: %v", lander.ErrInvalidRequest, uri, err)
		}
		return changeRef{
			Provider: scheme,
	...
	default:
		return changeRef{}, fmt.Errorf("%w: unsupported change URI scheme %q in %q", lander.ErrInvalidRequest, scheme, uri)
	}
}
