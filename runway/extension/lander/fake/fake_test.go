...
var testCfg = lander.Config{QueueName: "test-queue"}
...
func requestWith(uris ...string) *runwaymq.LandRequest {
...
func TestUnmarkedRequestSucceeds(t *testing.T) {
	...
	t.Run("check landability reports no outputs", func(t *testing.T) {
		res, err := New(testCfg, new(atomic.Uint64)).CheckLandability(context.Background(), req)
	...
	t.Run("land reports one output per step", func(t *testing.T) {
		res, err := New(testCfg, new(atomic.Uint64)).Land(context.Background(), req)
		require.NoError(t, err)
...
func TestMarkedRequestFails(t *testing.T) {
	tests := []struct {
		...
		{name: "conflict", token: tokenConflict, want: lander.ErrConflict},
		{name: "invalid", token: tokenInvalid, want: lander.ErrInvalidRequest},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, call := range []struct {
				name string
				fn   func(*runwaymq.LandRequest) (*runwaymq.LandResult, error)
			}{
				{"CheckLandability", func(r *runwaymq.LandRequest) (*runwaymq.LandResult, error) {
					return New(testCfg, new(atomic.Uint64)).CheckLandability(context.Background(), r)
				}},
				{"Land", func(r *runwaymq.LandRequest) (*runwaymq.LandResult, error) {
					return New(testCfg, new(atomic.Uint64)).Land(context.Background(), r)
				}},
			} {
				t.Run(call.name, func(t *testing.T) {
					...
					if tt.want != nil {
						...
						assert.True(t, lander.IsTerminal(err), "sentinel failures must be terminal")
						return
					}
					assert.False(t, lander.IsTerminal(err),
...
func TestFirstRecognizedTokenWins(t *testing.T) {
	...
	_, err := New(testCfg, new(atomic.Uint64)).Land(context.Background(), req)
	require.Error(t, err)
	assert.ErrorIs(t, err, lander.ErrConflict)
	assert.NotErrorIs(t, err, lander.ErrInvalidRequest)
}
