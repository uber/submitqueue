...
// mergerWithHeadBranchUpdates builds a merger that moves change head branches
// after a committing merge.
func (f gitFixture) mergerWithHeadBranchUpdates(t *testing.T, strategy mergestrategypb.Strategy) *gitMerger {
	t.Helper()
...
func TestMerge_Rebase_MovesHeadBranchToLandedCommit(t *testing.T) {
	...
	m := f.mergerWithHeadBranchUpdates(t, mergestrategypb.Strategy_REBASE)
...
func TestMerge_HeadBranchUntouchedWhenDisabled(t *testing.T) {
	f := setupGitFixture(t)
	head := f.pushPRCommit(t, "feature/a", "a.txt", "a\n", "add a")
...
func TestMerge_SquashRebase_MovesHeadBranchToSquashedCommit(t *testing.T) {
	f := setupGitFixture(t)
	...
	m := f.mergerWithHeadBranchUpdates(t, mergestrategypb.Strategy_SQUASH_REBASE)
...
func TestMerge_Rebase_MovesEachHeadBranchOfAStack(t *testing.T) {
	...
	res, err := m.Merge(context.Background(), req("b",
		stepOf(mergestrategypb.Strategy_REBASE, "s1", uriPR(1, first), uriPR(2, second)),
	))
	require.NoError(t, err)
...
func TestMerge_HeadBranchSkippedForForkChange(t *testing.T) {
...
func TestMerge_HeadBranchSkippedWhenAmbiguous(t *testing.T) {
...
func TestMerge_Merge_LeavesHeadBranchAlone(t *testing.T) {
	...
	m := f.mergerWithHeadBranchUpdates(t, mergestrategypb.Strategy_MERGE)
...
func TestMerge_HeadBranchUntouchedForAlreadyLandedChange(t *testing.T) {
...
func TestCheckMergeability_DoesNotMoveHeadBranch(t *testing.T) {
	...
	m := f.mergerWithHeadBranchUpdates(t, mergestrategypb.Strategy_REBASE)
	_, err := m.CheckMergeability(context.Background(), req("b", stepOf(mergestrategypb.Strategy_REBASE, "s1", uri(head))))
	require.NoError(t, err)
...
func TestUpdateHeadBranchFor_StaleLeaseFailsAndLeavesBranchAlone(t *testing.T) {
	...
	require.Error(t, err, "a refused lease must fail the merge, not be swallowed")
...
func TestMerge_HeadBranchMovesBeforeTheTargetIsPushed(t *testing.T) {
	...
	m := f.newMergerWith(t, func(p *Params) {
		p.DefaultStrategy = mergestrategypb.Strategy_REBASE
		p.UpdateHeadBranch = true
		p.MaxPushAttempts = 1
	})
	_, err := m.Merge(context.Background(), req("b", stepOf(mergestrategypb.Strategy_REBASE, "s1", uri(head))))
	require.Error(t, err, "the target push was rejected, so the merge failed")
...
func TestMerge_HeadBranchMovedAgainAfterTargetContention(t *testing.T) {
...
func TestMerge_HeadBranchPushFailureFailsTheLand(t *testing.T) {
	// Landing a change while knowing its head could not be moved produces exactly
	// the half-merged state the option exists to prevent, so the merge stops
...
func TestRemoteBranchTips(t *testing.T) {
	...
	// tip would otherwise make the merger rewrite the branch it just landed on.
