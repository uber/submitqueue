...
// the commit this merger last pushed that branch to. It is keyed by the commit
...
// trackedBranch is one change's resolved head branch and the commit this merger
// last pushed it to.
...
// same atomic push, both leave the provider comparing against the pre-merge head
...
//
// A failure to move a branch fails the merge, before the target is pushed.
// Landing a change while knowing its head could not be moved produces exactly
// the half-merged state the option exists to prevent, so the merge stops instead
...
// the merge carries on.
func (m *gitMerger) updateHeadBranches(ctx context.Context, updates []headUpdate, tracked headBranchTracker) error {
...
func (m *gitMerger) updateHeadBranchFor(ctx context.Context, u headUpdate, tips map[string][]string, tracked headBranchTracker) error {
	if u.newSHA == "" || u.newSHA == u.ref.SHA {
		return nil
	}
	...
	// batch was building fails this check, and the merge stops rather than
...
// the very branch the merge just landed on.
func (m *gitMerger) remoteBranchTips(ctx context.Context) (map[string][]string, error) {
