...
// It is the default a host wires before it has any integration, which keeps the
// dispatcher's behavior identical whether or not hooks are configured: events
// are still published, consumed, and acked, so turning a real hook on later
// changes only what happens to the event, not whether the seam works.
package noop
