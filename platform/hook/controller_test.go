...
func dispatcherDelivery(ctrl *gomock.Controller, payload []byte) *consumermock.MockDelivery {
...
func TestControllerProcess(t *testing.T) {
	t.Run("hands a well-formed event to the resolved hook", func(t *testing.T) {
		...
		require.NoError(t, newController(fixedHooks(h)).Process(context.Background(), dispatcherDelivery(ctrl, hookPayload(t, event))))
	...
	t.Run("a hook failure fails the delivery", func(t *testing.T) {
		...
		err := newController(fixedHooks(h)).Process(context.Background(), dispatcherDelivery(ctrl, hookPayload(t, validEvent(t))))
	...
	t.Run("an event no hook resolves to is acked", func(t *testing.T) {
		...
		require.NoError(t, newController(fixedHooks()).Process(context.Background(), dispatcherDelivery(ctrl, hookPayload(t, validEvent(t)))))
	})
}
...
func TestControllerRunsEveryResolvedHook(t *testing.T) {
	...
	err := newController(fixedHooks(export, comment, audit)).
		Process(context.Background(), dispatcherDelivery(ctrl, hookPayload(t, validEvent(t))))
...
func TestControllerRunsOnlyTheHooksResolvedForTheEvent(t *testing.T) {
	...
	require.NoError(t, newController(hooks).Process(context.Background(), dispatcherDelivery(ctrl, hookPayload(t, validEvent(t)))))
...
func TestControllerRejectsMalformedEvents(t *testing.T) {
	...
	for name, payload := range cases {
		t.Run(name, func(t *testing.T) {
			...
			require.Error(t, newController(fixedHooks(h)).Process(context.Background(), dispatcherDelivery(ctrl, payload)))
...
func TestControllerToleratesUnknownFields(t *testing.T) {
	...
	require.NoError(t, newController(fixedHooks(h)).Process(context.Background(), dispatcherDelivery(ctrl, payload)))
	require.Len(t, h.seen, 1)
}
