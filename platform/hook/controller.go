...
func (c *Controller) Process(ctx context.Context, delivery consumer.Delivery) error {
	...
	// Every hook runs even after one fails, so a broken integration cannot stop
	// the others from seeing the event. Failures carry the name of the hook that
	// raised them and are joined, so the classifier weighs each one and the
	// error says which integrations failed rather than that something did.
	var failures []error
	for _, h := range hooks {
		if err := h.Handle(ctx, event); err != nil {
			...
			failures = append(failures, fmt.Errorf("hook %s: %w", h.Name(), err))
		}
	}
