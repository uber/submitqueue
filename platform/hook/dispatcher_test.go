...
func newDispatcher(h *stubHook) *Dispatcher {
	return NewDispatcher(zap.NewNop().Sugar(), tally.NewTestScope("test", nil), h, testTopicKey, testGroup)
}

func TestDispatcherProcess(t *testing.T) {
	t.Run("hands a well-formed event to the hook", func(t *testing.T) {
		ctrl := gomock.NewController(t)
		h := &stubHook{}
		event := validEvent(t)

		require.NoError(t, newDispatcher(h).Process(context.Background(), dispatcherDelivery(ctrl, hookPayload(t, event))))
	...
	t.Run("a hook failure fails the delivery", func(t *testing.T) {
		...
		err := newDispatcher(h).Process(context.Background(), dispatcherDelivery(ctrl, hookPayload(t, validEvent(t))))
...
func TestDispatcherRejectsMalformedEvents(t *testing.T) {
	valid := validEvent(t)
	...
	for name, payload := range cases {
		t.Run(name, func(t *testing.T) {
			...
			require.Error(t, newDispatcher(h).Process(context.Background(), dispatcherDelivery(ctrl, payload)))
...
func TestDispatcherToleratesUnknownFields(t *testing.T) {
	ctrl := gomock.NewController(t)
	h := &stubHook{}
	payload := []byte(`{"id":"a/b/c/1","source":"a","type":"b","field_from_the_future":7}`)

	require.NoError(t, newDispatcher(h).Process(context.Background(), dispatcherDelivery(ctrl, payload)))
	require.Len(t, h.seen, 1)
}
