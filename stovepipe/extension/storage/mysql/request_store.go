...
func (r *requestStore) Get(ctx context.Context, id string) (ret entity.Request, retErr error) {
	...
	err := r.db.QueryRowContext(ctx,
		"SELECT id, queue, change_uri, state, version FROM request WHERE id = ?",
		id,
	).Scan(&req.ID, &req.Queue, &changeURIsJSON, &req.State, &req.Version)
...
// Create creates a new request. The request must have a unique ID already assigned. Returns ErrAlreadyExists if the request ID already exists.
func (r *requestStore) Create(ctx context.Context, request entity.Request) (retErr error) {
	...
	_, err = r.db.ExecContext(ctx,
		"INSERT INTO request (id, queue, change_uri, state, version) VALUES (?, ?, ?, ?, ?)",
		request.ID, request.Queue, changeURIsJSON, request.State, request.Version,
	)
	if err != nil {
...
// UpdateState updates the state of a request to newState and the version to newVersion
