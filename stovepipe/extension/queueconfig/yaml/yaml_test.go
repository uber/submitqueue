...
`,
		...
		{name: "unknown field", content: validQueueYAML("main", 1, 5000, 0) + "unexpected: true\n", wantErr: true},
		...
		{name: "negative minimum interval disables policy", content: validQueueYAML("main", 1, 5000, -1)},
		{
			name: "duplicate name",
			content: validQueueYAML("main", 1, 5000, 0) + `  - name: main
    max_concurrent: 1
    gate_wait_delay_ms: 5000
`,
			wantErr: true,
...
func validQueueYAML(name string, maxConcurrent int32, gateWaitDelayMs, minimumIntervalMs int64) string {
	return fmt.Sprintf(`queues:
  - name: %s
    max_concurrent: %d
    gate_wait_delay_ms: %d
    minimum_build_admission_interval_ms: %d
`, name, maxConcurrent, gateWaitDelayMs, minimumIntervalMs)
}

func TestStoreGetAndList(t *testing.T) {
	store, err := NewStore(writeQueueConfig(t, validQueueYAML("monorepo/main", 2, 5000, 3_600_000)))
	require.NoError(t, err)
