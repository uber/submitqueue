...
// validPayload is a change event as the upstream stage forwards it: a commit
// URI. The ordering key rides on the message envelope, not the payload.
...
func TestController_Process_PublishesToBatch(t *testing.T) {
	...
	// validate forwards the commit to batch, keyed by URI for idempotency and
