...
// captureRegistry builds a topic registry whose batch publisher records the
...
func captureRegistry(t *testing.T, ctrl *gomock.Controller, publishErr error, captured *entityqueue.Message) consumer.TopicRegistry {
	...
	registry, err := consumer.NewTopicRegistry([]consumer.TopicConfig{
		{Key: topickey.TopicKeyBatch, Name: "batch", Queue: mockQ},
	})
...
// validPayload is an ingest request as the upstream stage forwards it: identity,
// queue, and the change URIs. The ordering key rides on the message envelope, not
// the payload.
...
func TestController_Process_PublishesToBatch(t *testing.T) {
	ctrl := gomock.NewController(t)
	...
	// validate forwards the request to batch, keyed by spid for idempotency and
