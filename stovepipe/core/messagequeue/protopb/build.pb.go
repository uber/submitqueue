...
// BuildRequest is the payload process publishes to the build stage once it has
// admitted a request: only the request id travels on the queue. build reloads the
// full Request from storage by this id (producer and consumer share the store, so
// the id is enough and redelivery stays idempotent).
