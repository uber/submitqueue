...
func TestGetProjectStatusByURI(t *testing.T) {
	request := entity.Request{ID: projectStatusID, Queue: projectStatusQueue, URI: projectStatusURI, State: entity.RequestStateProcessing, Version: 1}
	tests := []struct {
		name            string
		...
		{name: "request visibility gap", request: entity.GetProjectStatusByURIRequest{Queue: projectStatusQueue, ChangeURI: projectStatusURI, Projects: []string{"project-a"}}, requestErr: storage.ErrNotFound, wantRetryable: true},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			requestStore := storagemock.NewMockRequestStore(mockCtrl)
			...
			if !tt.wantInvalid {
				...
				if tt.uriErr == nil {
					...
					requestStore.EXPECT().Get(gomock.Any(), projectStatusID).Return(request, tt.requestErr)
					if tt.requestErr == nil {
						store.EXPECT().GetRequestLogStore().Return(logStore)
						logStore.EXPECT().List(gomock.Any(), projectStatusID).Return([]entity.RequestLog{{State: request.State, RequestVersion: request.Version, TimestampMs: 1}}, nil)
...
func TestGetProjectStatusByURIRequiresCurrentLifecycleRecord(t *testing.T) {
	request := entity.Request{ID: projectStatusID, Queue: projectStatusQueue, URI: projectStatusURI, State: entity.RequestStateProcessing, Version: 2}
	mockCtrl := gomock.NewController(t)
	factory := storagemock.NewMockFactory(mockCtrl)
	store := storagemock.NewMockStorage(mockCtrl)
	uriStore := storagemock.NewMockRequestURIStore(mockCtrl)
	...
	store.EXPECT().GetRequestStore().Return(requestStore)
	requestStore.EXPECT().Get(gomock.Any(), projectStatusID).Return(request, nil)
	store.EXPECT().GetRequestLogStore().Return(logStore)
	logStore.EXPECT().List(gomock.Any(), projectStatusID).Return([]entity.RequestLog{{State: entity.RequestStateAccepted, RequestVersion: 1, TimestampMs: 1}}, nil)
...
func TestGetProjectStatusByURIRejectsInconsistentRequest(t *testing.T) {
	...
	uriStore.EXPECT().GetIDByURI(gomock.Any(), projectStatusURI).Return(projectStatusID, nil)
	store.EXPECT().GetRequestStore().Return(requestStore)
	requestStore.EXPECT().Get(gomock.Any(), projectStatusID).Return(entity.Request{ID: projectStatusID, Queue: projectStatusQueue, URI: "other"}, nil)
