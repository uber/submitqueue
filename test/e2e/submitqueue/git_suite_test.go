...
// runwayCommitter is the identity Runway records as committer on every commit
// it makes. The merger reads it back off a change's head branch to tell a
// branch it moved itself from one the change's author updated.
const (
	runwayCommitterName  = "SubmitQueue Runway"
	runwayCommitterEmail = "runway@submitqueue.invalid"
)
...
// A delivery that moved a change's head branch and then died before pushing the
// target leaves the branch on a commit Runway made, with the target where it
// was. The redelivery still pins the commit the branch has since left, so the
// change looks superseded while in fact nothing but Runway touched it.
//
// The lost delivery is reproduced by its effect rather than by killing Runway
// mid-push: what the redelivery sees is the ref state, and that is what this
// sets up.
func (s *GitMergeSuite) TestLand_HeadBranchMovedByLostDelivery_StillLands() {
	before := s.mainSHA()
	head := s.pushChange("feature/lost-delivery", map[string]string{"lost.txt": "lost\n"}, "add lost")
	updates := s.mainRefUpdateCount()

	movedTo := s.replayAsRunway("feature/lost-delivery", head)
	s.NotEqual(head, movedTo, "the head branch moved to the commit Runway made of the change")
	s.Equal(before, s.mainSHA(), "and the target never moved")
	s.Equal(updates, s.mainRefUpdateCount())

	s.requireStatus(s.land(gitQueue, s.uri("feature/lost-delivery", head)), entity.RequestStatusLanded)

	landed := s.mainSHA()
	s.NotEqual(before, landed, "the change landed rather than being rejected as stale")
...
// installMergeSignalFailOnce makes the first SUCCEEDED merge-signal publication
// for a batch fail, so the merge delivery is redelivered with Git already
// updated. That is the only way, from outside the process, to stop a controller
// between two of its own steps.
//
// Doing it with a trigger ties the helper to the MySQL queue backend: it needs a
// server that supports triggers, a non-transactional table so the counters
// survive the rolled-back insert, and SIGNAL to raise an error the publisher
// treats as retryable. A queue on any other backend would need its own
// injection mechanism, and this test would not run against it.
//
// TODO(e2e): give the harness a backend-independent way to fail a controller
// partway — a fault-injection seam the consumer honours — so a test like this
// stops depending on the queue's storage engine.
...
// replayAsRunway replays a change onto the current target under Runway's
// committer identity and moves the change's branch there, without touching the
// target. That is exactly the state a delivery leaves behind when it updates
// head branches and then fails before pushing the target.
func (s *GitMergeSuite) replayAsRunway(branch, sha string) string {
	s.runGit(s.work, "fetch", "origin")
	s.runGit(s.work, "checkout", "-B", "runway-replay", "origin/main")
	s.runGit(s.work,
		"-c", "user.name="+runwayCommitterName,
		"-c", "user.email="+runwayCommitterEmail,
		"cherry-pick", sha,
	)
	replayed := strings.TrimSpace(s.runGit(s.work, "rev-parse", "HEAD"))
	s.runGit(s.work, "push", "--force", "origin", replayed+":refs/heads/"+branch)
	s.runGit(s.work, "checkout", "main")
	return replayed
}
