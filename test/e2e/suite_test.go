...
func (s *E2EIntegrationSuite) TestLandRequest() {
	...
	require.NoError(s.T(), err, "failed to query request from database")
	assert.Equal(s.T(), "new", state, "request should be in 'new' state")

	s.log.Logf("Verified request %s is in database with state=%s", resp.Sqid, state)
}
