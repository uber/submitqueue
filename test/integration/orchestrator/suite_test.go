...
//
// These tests use docker-compose from example/server/orchestrator/docker-compose.yml
...
func (s *OrchestratorIntegrationSuite) SetupSuite() {
	...
	// Use docker-compose from example/server/orchestrator
	// NOTE: Assumes Linux binary is pre-built via make target
	composeFile := filepath.Join(repoRoot, "example/server/orchestrator/docker-compose.yml")
	s.stack = testutil.NewComposeStack(t, s.log, s.ctx, composeFile, "svc-orchestrator")
