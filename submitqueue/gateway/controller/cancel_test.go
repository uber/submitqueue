...
func newCancelStorage(t *testing.T, ctrl *gomock.Controller, logStore storage.RequestLogStore, summaryStore storage.RequestSummaryStore) storage.Storage {
	t.Helper()
	contextStore := storagemock.NewMockRequestContextStore(ctrl)
	contextStore.EXPECT().Get(gomock.Any(), gomock.Any()).Return(entity.RequestContext{Queue: "my-queue"}, nil).AnyTimes()
	store := storagemock.NewMockStorage(ctrl)
	store.EXPECT().GetRequestLogStore().Return(logStore).AnyTimes()
	store.EXPECT().GetRequestSummaryStore().Return(summaryStore).AnyTimes()
	store.EXPECT().GetRequestContextStore().Return(contextStore).AnyTimes()
...
func TestCancel_InsertsCancellingLog(t *testing.T) {
	...
	summaryStore := storagemock.NewMockRequestSummaryStore(ctrl)
	summaryStore.EXPECT().Get(gomock.Any(), "my-queue", "my-queue/42").Return(entity.RequestSummary{Version: 1}, nil)
	summaryStore.EXPECT().Update(gomock.Any(), gomock.Any(), int64(1), int64(2)).Return(nil)
