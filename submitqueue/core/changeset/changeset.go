...
// consolidating controller-side and backend-specific traversal.
...
// operate on a single batch — callers with several batches loop and keep the
// per-batch boundary by holding a slice per batch.
...
type Resolver interface {
	...
	// with no requests yields an empty slice. Used by build runners for their
	// base and head batches.
