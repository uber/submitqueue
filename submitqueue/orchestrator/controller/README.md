...
Do not add an unbounded retry loop inside a controller. On `storage.ErrVersionMismatch`, return the wrapped sentinel and let the shared domain classifier mark it retryable. Redelivery then starts from a fresh read and decides whether the transition is still needed, already happened, or was superseded.
...
5. Does a version conflict return for classified redelivery rather than spin locally?
6. Which controller or DLQ path owns superseded and terminal recovery?
