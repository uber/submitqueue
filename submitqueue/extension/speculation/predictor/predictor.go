...
// it changes and what has happened to it so far. A Scorer prices the change; an
// OutcomePredictor is built over one and revises its price with the batch's
// observed progress.
package predictor
...
// OutcomePredictor estimates a batch's final outcome.
...
type Config struct {
	// QueueName identifies the queue this OutcomePredictor serves.
	QueueName string
}

// Factory builds the OutcomePredictor for a queue. Implementations inject what
// they need at construction, including the Scorer whose price they revise.
type Factory interface {
	// For returns the OutcomePredictor for the given queue.
	For(cfg Config) (OutcomePredictor, error)
}
