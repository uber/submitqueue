...
func newTestLandChecker(t *testing.T, serverURL string) landchecker.LandChecker {
	t.Helper()
	client, err := phttp.NewClient(serverURL)
	require.NoError(t, err)
	return NewLandChecker(Params{
		HTTPClient:   client,
...
func TestLandChecker_Check(t *testing.T) {
	tests := []struct {
		name         string
	...
	}{
		{
			...
			change:       change.Change{URIs: []string{"github://github.example.com/uber/repo/pull/1/" + shaAFull}},
			wantLandable: true,
		},
		{
		...
		},
		{
			...
			change:       change.Change{URIs: []string{"github://github.example.com/uber/repo/pull/1/" + sha1Full, "github://github.example.com/uber/repo/pull/2/" + sha2Full}},
			wantLandable: true,
		},
		{
			name: "unknown landability returns error",
			handler: graphQLHandler(t, []PRInfo{
		...
		},
		{
			...
			change:       change.Change{URIs: []string{"github://github.example.com/uber/repo/pull/1/" + shaOldFull}},
			wantLandable: false,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mc := newTestLandChecker(t, server.URL)
			...
			require.NoError(t, err)
			assert.Equal(t, tt.wantLandable, result.Landable)
