...
- `buildkite`: Buildkite-backed backend. Its HTTP client and Buildkite-specific facts (state vocabulary, metadata env-var round-trip) live in [`platform/buildkite`](../../../platform/buildkite/README.md), shared with `stovepipe`'s own Buildkite backend.
