...
func TestRequestSummaryStore_Get(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := store.Get(context.Background(), tt.requestID)
			if tt.wantErr {
				require.Error(t, err)
				if tt.wantErrIs != nil {
					assert.ErrorIs(t, err, tt.wantErrIs)
...
func TestNormalizeChangeURIs(t *testing.T) {
	assert.Equal(t, []string{}, normalizeChangeURIs(nil))
	assert.Equal(t, []string{"a"}, normalizeChangeURIs([]string{"a"}))
}

func TestNormalizeMetadata(t *testing.T) {
	assert.Equal(t, map[string]string{}, normalizeMetadata(nil))
	assert.Equal(t, map[string]string{"a": "b"}, normalizeMetadata(map[string]string{"a": "b"}))
}
