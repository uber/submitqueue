...
// Insert appends a new request log record. The primary key is (queue, request_id, timestamp_ms, salt).
...
func (r *requestLogStore) Insert(ctx context.Context, log entity.RequestLog) (retErr error) {
	...
	if log.Queue != r.queue {
		return fmt.Errorf("request log request_id=%s queue %q does not match the store's bound queue %q", log.RequestID, log.Queue, r.queue)
	}
	...
	// (queue, request_id, timestamp_ms). The salt is part of the composite primary key in MySQL
...
// List retrieves all request log records for a given request ID within the bound queue,
// ordered by timestamp ascending. Salt is used as a secondary sort key to provide stable
// ordering for entries that share a timestamp, but it is not included in the SELECT columns
...
func (r *requestLogStore) List(ctx context.Context, requestID string) (ret []entity.RequestLog, retErr error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to list request logs for queue=%s request_id=%s: %w", r.queue, requestID, err)
	}
	defer rows.Close()
	...
	if len(logs) == 0 {
		return nil, fmt.Errorf("no request log records for queue=%s request_id=%s: %w", r.queue, requestID, storage.ErrNotFound)
	}
