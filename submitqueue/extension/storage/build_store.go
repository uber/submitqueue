...
type BuildStore interface {
	...
	// GetByBatchID retrieves the build scheduled for the given batch. In the
	// current model there is at most one build per batch (build.ID is the
	// runner-assigned ID, so callers that only hold a batch ID cannot use Get).
