...
const changeIDFormat = "git://{host}/{repo-path}/{ref}/{commit_sha}"

// shaLength is the length of a git commit SHA.
const shaLength = 40

// ChangeID represents a parsed git:// change identifier.
// Format: git://{host}/{repo-path}/{ref}/{commit_sha}
//
...
type ChangeID struct {
	...
	Scheme string
	// Host is the git remote authority, "host" or "host:port" (e.g.
	// "git.example.com" or "git.example.com:9418"). Unlike a forge "org", git
	// addresses a repository by the remote it lives on.
	Host string
	// RepoPath is the path to the repository on the remote and may contain
	// slashes (e.g. "uber/monorepo" or "team/group/repo.git").
	RepoPath string
	// Ref is the fully-qualified git ref the change landed on, decoded from the
	// URI (e.g. "refs/heads/main", "refs/tags/v1.0").
	Ref string
	// CommitSHA is the commit Ref pointed at, captured at a point in time. It is
	// not necessarily a branch head; it records the commit Ref resolved to when
	// the change entered the pipeline.
	CommitSHA string
}

// ParseChangeID parses a raw change ID string into a ChangeID.
// Expected format: git://{host}/{repo-path}/{commit_sha} with a fully-qualified,
// percent-encoded ref as the second-to-last path segment.
func ParseChangeID(raw string) (ChangeID, error) {
	...
	if u.Host == "" {
		return ChangeID{}, fmt.Errorf("invalid change ID %q: missing remote host (expected format: %s)", raw, changeIDFormat)
	}
	...
	// Need at least 3 segments: {repo-path}/{ref}/{commit_sha}.
	if len(segments) < 3 {
		return ChangeID{}, fmt.Errorf("invalid change ID %q: need at least repo-path/ref/sha, got %d path segments (expected format: %s)", raw, len(segments), changeIDFormat)
	}

	sha := segments[len(segments)-1]
	encodedRef := segments[len(segments)-2]
	repoPath := strings.Join(segments[:len(segments)-2], "/")
	...
	if repoPath == "" {
		return ChangeID{}, fmt.Errorf("invalid change ID %q: empty repo path (expected format: %s)", raw, changeIDFormat)
	}
...
func (c ChangeID) String() string {
	return fmt.Sprintf("%s://%s/%s/%s/%s", c.Scheme, c.Host, c.RepoPath, url.PathEscape(c.Ref), c.CommitSHA)
}
