...
func TestLandRequestRoundTrip(t *testing.T) {
	req := &LandRequest{
		Id:        "queue-a/42",
		QueueName: "queue-a",
		Steps: []*LandStep{
	...
	assert.True(t, proto.Equal(req, got), "round-tripped LandRequest should equal the original")
}

func TestLandResultRoundTrip(t *testing.T) {
	// A committing land reports the revisions each step produced on the target;
	// a dry-run check leaves outputs empty and reports a per-step reason on
	// failure. Both shapes share the one LandResult contract.
	cases := map[string]*LandResult{
		"landed with produced revisions": {
			Id:      "queue-a/42",
	...
	for name, res := range cases {
		t.Run(name, func(t *testing.T) {
			...
			got := &LandResult{}
			require.NoError(t, Unmarshal(data, got))
			assert.True(t, proto.Equal(res, got), "round-tripped LandResult should equal the original")
		})
	}
}
...
func TestWireFormat(t *testing.T) {
	data, err := Marshal(&LandRequest{
		Id:        "queue-a/42",
		QueueName: "queue-a",
		Steps:     []*LandStep{{StepId: "queue-a/1", Strategy: strategypb.Strategy_SQUASH_REBASE}},
	})
	require.NoError(t, err)
...
func TestTopicKeysBindEveryTopicKey(t *testing.T) {
	...
	for _, m := range []proto.Message{&LandRequest{}, &LandResult{}} {
		keys := TopicKeys(m)
