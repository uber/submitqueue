...
// payloads for the land queues Runway owns, defined by the proto files in
...
//
// One contract serves two queue pairs because a land-conflict check is a dry
// run of a land: Runway applies the same ordered steps onto the same target
...
// — the land-conflict-check pair for a dry run, the land pair for a
// committing land — so LandRequest and LandResult are identical on both.
...
type (
	...
	// queues: the land-conflict-check topic for a dry-run check, the land
	// topic for a committing land.
	LandRequest = protopb.LandRequest
	// LandStep is one step of an ordered land: a single set of change(s)
	// applied with a strategy.
	LandStep = protopb.LandStep
	// LandResult is the payload Runway publishes to the corresponding signal
	// queue once a request completes.
	LandResult = protopb.LandResult
	// StepResult reports what happened to a single LandStep.
	StepResult = protopb.StepResult
	// StepOutput is a single revision a land step produced on the land target.
